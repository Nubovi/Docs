...
$roleName = "Nubovi Reader Custom Role"
$appName = "Nubovi Reader Custom"
...
# initialize registered actions and actions
$registeredActions = @()
$actions = @(
...
$providers = az provider list --query "[?registrationState=='Registered'].{Provider:namespace,Status:registrationState}" --output json | ConvertFrom-Json

foreach ($provider in $providers) {
    $namespace = $provider.Provider
    # Iterate over each action in the predefined list
    foreach ($action in $actions) {
        # Check if the provider name matches the provider name in the action string
        if ($action -match "^$namespace") {
            # If the provider name matches, add the action to the list of registered actions
...
$app=(az ad sp create-for-rbac -n $appName | ConvertFrom-Json) 
